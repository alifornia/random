...
deaths.rename(columns = {'Reporting Area':'area'}, inplace = True)
deaths.rename(columns = {'MMWR WEEK':'WEEK'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), All Ages**':'all'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), LT 1':'LT 1'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), 1–24':'1-24'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), 25–44':'25-44'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), 45–64':'45-64'}, inplace = True)
deaths.rename(columns = {'All causes, by age (years), ≥65':'65+'}, inplace = True)
deaths.rename(columns = {'P&I† Total':'total'}, inplace = True)


#Big Areas Total (seasons)
big_areas =['New England', 'Mid. Atlantic', 'E.N. Central', 'W.N. Central', 'S. Atlantic', 'E.S. Central', 'W.S. Central', 'Mountain', 'Pacific']
deaths_season = deaths.copy()
deaths_season = deaths_season[(deaths_season['area'].isin(big_areas))]
...
for i in range(1,5):
    deaths_season['SEASON'+str(i)] = deaths_season[(deaths_season['WEEK']>10*(i-1)) & (deaths_season['WEEK']<=10*i)].groupby('area')['all'].transform(sum)
deaths_season = deaths_season[['area', 'SEASON1', 'SEASON2', 'SEASON3', 'SEASON4']]
deaths_season=deaths_season.drop_duplicates()
deaths_season = deaths_season.fillna(0)
deaths_season = deaths_season.groupby(['area'])[['SEASON1','SEASON2','SEASON3','SEASON4']].sum()
...
deaths_age = deaths.copy()
deaths_age = deaths_age[(deaths_age['area'].isin(big_areas))]
