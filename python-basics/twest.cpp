...
class DirDependenciesOwners
{ // maybe for larger OO design but not needed for this quesion
  public:
...
void TwitterCodebaseApprovalSystem::process_program_options(const int ac, const char *const av[])
{
    ...
    desc.add_options()("help,h", "produce help message")
        ...
        ("approvers,a", po::value(&approvers)->multitoken(), "List of approvers")("changed-files,c", po::value(&changed_files)->multitoken(), "List of changed files");
    try
    {
        po::variables_map vm;
...
void TwitterCodebaseApprovalSystem::get_dir_owners(string changed_file, hashmap_v &dir_direct_owners)
{
    ...
    while (!stack.empty())
    { //DFS to get all owners: direct or through dependencies
        dir_path_str = stack.top();
        stack.pop();

        if (visited_dir.find(dir_path_str) == visited_dir.end())
